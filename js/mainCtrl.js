angular.module('itunes').controller('mainCtrl', function($scope, itunesService){
  ...
  $scope.songData = [];
  $scope.filters = [
    // Music, Music Videos, TV Shows, Movies, Podcasts, All
    {
        name: 'Music'
      , value: 'song'
    } ,
  ...
  $scope.gridOptions = {
      data: 'songData',
      height: '110px',
      sortInfo: {fields: [ 'Song', 'Artist', 'CollectionName' ], directions: ['asc']},
  ...
  //Also note that that method should be retuning a promise, so you could use .then in this function.
    ...
    $scope.getSongData = function( artist ) {
      itunesService.getArtistInfo( artist ).then( function( results ) {
        var searchResults = [];
            ...
            searchResults.push(
              {
                ...
                , TrackPrice: '$' + results[ i ].trackPrice
                , Play: results[ i ].previewUrl
                , Genre: results[ i ].primaryGenreName
                , CollectionName: results[ i ].collectionName
